...
class DrawAttention_CustomFields {
	...
	function __construct( $parent ) {
		...
		add_filter( 'cmb2_meta_boxes', array( $this, 'highlight_define_styles_metabox' ), 2000 );
	...
	function highlight_styling_metabox( $metaboxes ) {
		...
		$metaboxes['highlight_styling'] = array(
			...
			'fields' => array(
				...
				array(
					'name'    => __( 'Always show hotspots for this image', 'draw-attention' ),
				...
				array(
					'name'    => __( 'Hover Color', 'draw-attention' ),
					'desc'    => '',
				...
				array(
					'name'    => __( 'Hover Opacity', 'draw-attention' ),
					'desc'    => '',
				...
				array(
					'name'    => __( 'Define multiple highlight styles for this image', 'draw-attention' ),
					'desc'    => __( 'Use multiple highlight colors on this image', 'draw-attention' ),
	...
	function moreinfo_metabox( $metaboxes ) {
		...
		$metaboxes['moreinfo'] = array(
			'id' => 'moreinfo_metabox',
			'title' => __( 'General Settings', 'draw-attention' ),
			'object_types' => array( $this->parent->cpt->post_type, ),
			'context'       => 'normal',
			'priority'      => 'high',
			'fields' => array(
				...
				array(
					...
					'desc'    => __( 'Set the background color behind the image', 'draw-attention' ),
	...
	function highlight_define_styles_metabox( $metaboxes ) {
		if ( ! is_array( $metaboxes ) ) {
			$metaboxes = array();
		}
		...
		$metaboxes['styles'] = apply_filters( 'highlight_define_styles', array(
			'id'           => 'styles',
			'title'        => __( 'Styles', 'draw-attention' ),
			'object_types' => array( $this->parent->cpt->post_type, ),
			'fields'       => array(
				array(
					'id'          => $this->prefix . 'styles',
					'type'        => 'group',
					'description' => __( 'Define the alternate styles you\'d like to use', 'draw-attention' ),
					'options'     => array(
						'group_title'   => __( 'Style #{#}', 'draw-attention' ), // {#} gets replaced by row number
						'add_button'    => __( 'Add Another Style', 'draw-attention' ),
						'remove_button' => __( 'Remove Style', 'draw-attention' ),
						'sortable'      => false, // beta
