...
<style>
	...
	#<?php echo $settings['spot_id']; ?> .bc-product__title a,
	.featherlight .featherlight-content.lightbox-<?php echo $settings['image_id']; ?> .hotspot-title,
	.featherlight .featherlight-content.lightbox-<?php echo $settings['image_id']; ?> .bc-product__title a {
			...
			<?php foreach( $settings['hotspots'] as $key => $hotspot ) : ?>
				<?php
					...
					if ( 0 === strpos( $target_url, 'http://[' ) ) {
						// Awkwardly support shortcodes
						$target_url = str_replace( 'http://[', '[', $target_url);

						$target_url = do_shortcode( urldecode( $target_url ) );
					}
					$rel = '';
					...
					if ( empty( $settings['img_settings']['_da_has_multiple_styles']['0'] ) || $settings['img_settings']['_da_has_multiple_styles']['0'] != 'on' || empty( $hotspot['style'] ) ) {
						...
						$color_scheme_exists = false;
						foreach ($formatted_styles as $style) {
							if ( $style['name'] == $hotspot['style'] ) {
				...
				<area
					...
					data-id="<?php echo 'area-' . $settings['spot_id'] . '-' . $key; ?>"
